...
final class PusherTransportTest extends TransportTestCase
{
    ...
    {
        return new PusherTransport('xoxb-TestToken', $channel, $client ?? $this->createMock(HttpClientInterface::class));
    }

    public function toStringProvider(): iterable
    {
        yield ['pusher://pusher.com', $this->createTransport()];
        yield ['pusher://pusher.com?channel=test+Channel', $this->createTransport(null, 'test Channel')];
    }
    ...
    public function testInstatiatingWithAnInvalidSlackTokenThrowsInvalidArgumentException()
    {
        ...
        $this->expectExceptionMessage('A valid Pusher token needs to start with "xoxb-", "xoxp-" or "xoxa-2". See https://api.pusher.com/authentication/token-types for further information.');

        new PusherTransport('token', 'testChannel', $this->createMock(HttpClientInterface::class));
    }
    ...
    public function testSendWithErrorsIncluded()
    {
        ...
        $this->expectException(TransportException::class);
        $this->expectExceptionMessage('Unable to post the Pusher message: "invalid_blocks" (no more than 50 items allowed [json-pointer:/blocks]).');
